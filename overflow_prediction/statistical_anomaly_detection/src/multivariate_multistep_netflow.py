...
def handle_netflow(data_path, data_file, folder_name, output_path, lstm_units, optimizer, loss, epochs,
    ...
    X, y, dates_X, dates_y, threshold, _ = dp.preprocess_multistep_netflow_data([data_path + data_file], n_steps,
                                                                                n_steps_out, slide_len, 2,
                                                                                overflow_thresh)
    vols = {}
    lm.multistep_multivariate_lstm(batch_size, epochs, folder_name, loss, lstm_units, n_features, n_steps, optimizer,
                                   output_path, rep, slide_len, timestamp, vols, csv_logger, l_rate, n_steps_out, X, y,
                                   dates_X, dates_y, ex, threshold, lstm_layers, use_mini_batches)
