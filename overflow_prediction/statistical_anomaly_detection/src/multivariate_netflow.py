...
def handle_netflow(data_path, data_file, folder_name, output_path, lstm_units, optimizer, loss, epochs,
                   batch_size, n_steps, slide_len, timestamp, rep, l_rate, n_steps_out, overflow_thresh, lstm_layers,
                   use_mini_batches):
    if not os.path.exists(output_path + folder_name + '//' + str(int(timestamp))):
        os.mkdir(output_path + folder_name + '//' + str(int(timestamp)))
    X, y, dates_X, dates_y, threshold, all_data = dp.preprocess_multistep_netflow_data([data_path + data_file], n_steps,
                                                                                       1, slide_len, 0, overflow_thresh)
    # df = pd.read_csv(data_path + data_file)
    # vols = {}
    # for index, row in df.iterrows():
    #     vols[row['id']] = [row['ts'], row['vol']]
    lm.multivariate_lstm(batch_size, epochs, folder_name, loss, lstm_units, n_steps, optimizer,
                         output_path, rep, slide_len, timestamp, l_rate, n_steps_out, X, y, dates_X, dates_y, ex,
                         lstm_layers, all_data, threshold, use_mini_batches)


def main(folder_name, data_file, data_path, output_path, lstm_units_list, optimizers, losses, epochs_list,
         batch_sizes, n_steps_list, n_features_list, slide_lens, repetitions, n_steps_out, l_rates, overflow_thresholds,
         lstm_layers, use_mini_batches):
